...
$.ddMM.mm_widget_showimagetvs = {
	...
	templates: {
		previewContainer: '<div class="tvimage"><img src="" style="display: none; max-width: [+cs_width+]px; max-height: [+cs_height+]px; margin: 4px 0; cursor: pointer;" /></div>'
	}
};
...
$.fn.mm_widget_showimagetvs = function(params){
	...
	return $(this).addClass('imageField').each(function(){
		...
		$previewContainer.appendTo($this.parent());
		...
		var $this = $(this),
			// Get the new URL
			url = $.trim($this.val()),
			$img = $this.parent().find('.tvimage img');
		...
		if (url.length > 0 && params.thumbnailerUrl.length > 0){
			url = params.thumbnailerUrl + '?src=' + escape(url) + '&w=' + params.cs_width + '&h=' + params.cs_height;
		}
